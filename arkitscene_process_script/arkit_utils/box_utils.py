...
def generate_bbox_voxel(size,center,rot_mat):
    l, h, w = [i / 2 for i in size]
    center = np.reshape(center, (-1, 3))
    center = center.reshape(3)
    x=np.linspace(start=-l,stop=l,num=10)
    y=np.linspace(start=-h,stop=h,num=10)
    z=np.linspace(start=-w,stop=w,num=10)

    X,Y,Z=np.meshgrid(x,y,z,indexing='ij')

    voxel_coor=np.concatenate([X[:,:,:,np.newaxis],Y[:,:,:,np.newaxis],Z[:,:,:,np.newaxis]],axis=-1)
    voxel_coor=np.reshape(voxel_coor,(10*10*10,3))
    voxel_rot=np.dot(voxel_coor,rot_mat)
    #print(voxel_rot.shape,center.shape,center[0])
    voxel_rot[:,0]+=center[0]
    voxel_rot[:,1]+=center[1]
    voxel_rot[:,2]+=center[2]
